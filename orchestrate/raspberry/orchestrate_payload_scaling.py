...
RESULT_DIRECTORY = PROJECT_DIRECTORY / "results" / "payload-scaling"
...
def load_environment_variables():
    ...
    os.environ["PAYLOAD_SCALING_RESULT_DIR"] = str(RESULT_DIRECTORY)

    PAYLOAD_SIZES = os.environ["PAYLOAD_SCALING_PAYLOAD_SIZES"].split(",")

    BENCHMARK_RUNS = int(os.environ["PAYLOAD_SCALING_RUNS"])
...
def build_binaries():
    ...
    subprocess.run(
        ["go", "test", "-c", "-o", BENCHMARK_BINARY, "./micro/payload_scaling"],
...
def run_benchmark(
    ...
    benchmark_name = (
        f"^BenchmarkPayloadScaling{operation}$/"
        f"^{group}$/"
        f"^{sweep_value}B$"
    )
...
def run_timing_benchmarks():
    ...
    print("Phase 1 of 3 - PSK payload scaling")
    ...
    print("Phase 2 of 3 - RSA + AES payload scaling")
    ...
    print("Phase 3 of 3 - CP-ABE + AES payload scaling")
...
def generate_report():
    ...
    print("Generating Payload Size Scaling HTML report...")
