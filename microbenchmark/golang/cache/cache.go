...
func StoreFile(fileName string, fileContent []byte) {
	...
	if err := os.MkdirAll(cacheDirectory, utils.DirectoryPermissions); err != nil {
		panic(err)
	}

	if err := os.WriteFile(getFilePath(fileName), fileContent, utils.FilePermissions); err != nil {
		panic(err)
	}
}
